...
// https://aviationweather.gov/api/data/metar?format=xml&hoursBeforeNow=3&mostRecentForEachStation=true&ids=KCLL,KOCF
...
#define WIND_THRESHOLD 14        // Winds or gusting winds above this but less than HIGH_WIND_THRESHOLD cause LED to blink between black/clear and the flight category color
...
#define DO_WINDS true            // Causes LED to blink either back or orange if winds exceed WIND_THRESHOLD or HIGH_WIND_THRESHOLD
                                 // Note that this must be true to blink for lightning, too!
  ...
  if (DO_WINDS && windLeds.size() > 0) {
    ...
    for (unsigned short int i = 0; i < windLeds.size(); ++i) {
      ...
      if (FADE_FOR_HIGH_WINDS)
        leds[currentLed] %= 128;  //fade by 50%, never fading to black
      else
